...
class MinHeap::MinHeapImpl {
 private:
  ...
  int GetSmallerIndex(int current_index, int new_index) {
    if (new_index < this->heap.size() && this->heap[new_index].first < this->heap[current_index].first) {
      return new_index;
    }

    // Assume current_index is always within vector
    if (this->heap[current_index].first < this->heap[new_index].first) {
      return current_index;
    }

    // If the keys are the same, return the newer version
    if (this->heap[new_index].first == this->heap[current_index].first &&
        this->heap[new_index].second > this->heap[current_index].second) {
      return new_index;
    }
    return current_index;
  }
 ...
 public:
  explicit MinHeapImpl(std::vector<K> initial_keys) {
    ...
    for (int i = 0; i < this->heap.size(); i++) {
      this->heap[i] = std::make_pair(initial_keys[i], i);
    }
  ...
  std::optional<std::pair<K, int>> InsertAndExtract(std::pair<K, int> next_pair) {
    ...
    std::pair<K, int> previous_min = this->heap[0];
    this->heap[0] = next_pair;
    this->HeapifyDown(0);
    return previous_min;
  }
...
std::optional<std::pair<K, int>>  MinHeap::InsertAndExtract(std::pair<K, int> next_pair) {
